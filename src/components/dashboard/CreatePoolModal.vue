<template>
  <div class="modal-overlay">
    <div class="modal">
        <p id="heading-create"> <b> Create Pool Form </b> </p>
        <form id="create-pool-form" @submit.prevent>
            <label class="modal-texts p-name"> <b> Pool Name </b></label>
            <input type="text" class="rm-shad p-shad" v-model="poolName"><br><br>
            <label class="modal-texts"> <b>Members List</b> </label>
            ...
            <ol>
                <li id="members-list" v-for="(member,index) in membersList" :key="index">
                    <label class="modal-texts l-name"> Email </label>
            ...
            <div id="sbtn-div">
            <Button class="submit-btn" :disabled="shouldDisableCreateBtn" @click="createPool"> <b>Create</b> </Button>
            </div>
        </form>
    </div>
  ...
  export default {
      ...
      computed: {
          shouldDisableCreateBtn() {
              ...
              return isAtleastOneElementEmpty || this.poolName == ""
          }
      },
      ...
      methods: {
          ...
          async createPool() {
              this.$store.wasCreatePoolApiHit = true;
              let myPoolName = this.poolName;
              let myMembersList =  this.membersList;
              this.poolName = ""
              this.membersList = [""]

              const apiPath = HOST + "/pool/create"

              let arr = []
              for(let i = 0; i < myMembersList.length; i++) {
                  let x = {"email":myMembersList[i]}
                  arr.push(x);
              }
             ...
             headers.append('Content-Type', 'application/json'); // Set the content type to JSON
              ...
              try {
                    ...
                    if(responseData.status == "SUCCESS") {
                        this.$notify("Pool created with id: " + responseData.poolId);
                        return;
                    }

                    this.$notify("Unknown error occured" + responseData)
